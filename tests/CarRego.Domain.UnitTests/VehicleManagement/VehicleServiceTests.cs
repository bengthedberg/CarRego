...
using CarRego.Domain.VehicleManagement;
using CarRego.Domain.VehicleManagement.Data;
using CarRego.Domain.VehicleManagement.Exceptions;

namespace CarRego.Domain.UnitTests.VehicleManagement;

public class VehicleServiceTests : TestBase<VehicleManagementContext, VehicleService>
{
    ...
    public class GetVehicleByVin
    {
        ...
        [Fact]
        public Task Returns_InvalidVinException_if_VIN_is_invalid()
            => RunTest(
                    service: x => new VehicleService(x),
                    prepare: cmd => Task.CompletedTask,
                    execute: async svc =>
                    {
                        var result = await svc.GetVehicleByVin(INVALID_VIN);
        ...
        public Task Returns_VehicleDTO_if_ID_exists()
            => RunTest(
                    ...
                    execute: async svc =>
                    {
                        ...
                        Assert.Equal("John", owner.FirstName);
                        Assert.Equal("Doe", owner.LastName);
                        Assert.Equal(new DateTime(DateTime.Today.AddYears(-10).Ticks, DateTimeKind.Unspecified), owner.From);
                        Assert.Null(owner.To);
        ...
        public Task Returns_VehicleOwnerDTO_if_VIN_exists()
            => RunTest(
                    ...
                    execute: async svc =>
                    {
                        ...
                        Assert.Equal(DateTime.Today.AddYears(-8).ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), result.Result!.From.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
