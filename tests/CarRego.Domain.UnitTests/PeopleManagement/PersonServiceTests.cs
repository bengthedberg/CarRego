using CarRego.Domain.PeopleManagement;
using CarRego.Domain.PeopleManagement.Data;
using CarRego.Domain.PeopleManagement.Exceptions;
using CarRego.Domain.UnitTests.Extensions;
using CarRego.Domain.UnitTests.Infrastructure;

namespace CarRego.Domain.UnitTests.PeopleManagement;

[Collection("Sequential")]
public class PersonServiceTests : TestBase<PeopleManagementContext, PersonService>
{
    public static (string Type, string AddressLine1, string AddressLine2, string PostalCode, string City, string Country, bool IsCurrent) GetAddress(
        string Type, string AddressLine1, string AddressLine2, string PostalCode, string City, string Country, bool IsCurrent
    ...
    public class GetPersonById
    {
            ...
            => RunTest(
                    ...
                    prepare: async cmd =>
                    {
                        await cmd.AddPerson(1, "John", "Doe", new[] {
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true)
                        });
                    },
                    execute: async svc =>
                    {
        ...
        public Task Includes_current_addresses_in_response()
            => RunTest(
                    ...
                    {
                        await cmd.AddPerson(1, "John", "Doe", new[] {
                            GetAddress(nameof(DeliveryAddress), "c/o Jimmy Doe", "Street 2", "98765", "The Other City", "Denmark", false),
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true)
                        });
                    },
                    execute: async svc =>
        ...
        public Task Returns_all_addresses_in_AddressDTO_array_if_addresses_exist()
            => RunTest(
                    ...
                    prepare: async cmd =>
                    {
                        await cmd.AddPerson(1, "John", "Doe", new[] {
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(InvoiceAddress), "c/o Jimmy Doe", "Street 2", "98765", "The Other City", "Denmark", false)
                        });
                    },
                    execute: async svc =>
                    {
    ...
    {
        [Fact]
        public Task Adds_new_delivery_address_to_database_and_returns_AddressDTO()
            => RunTest(
                    service: x => new PersonService(x),
                    prepare: async cmd =>
                    {
                    ...
                    execute: async svc =>
                    {
                        ...
                        Assert.NotNull(result);
                        Assert.True(result.IsSuccess);
                        Assert.Equal("Delivery", result.Result!.Type);
                    ...
                    validate: async cmd =>
                    {
                        ...
                        using (var rs = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            Assert.Equal(nameof(DeliveryAddress), rs["Type"]);
        ...
        [Fact]
        public Task Adds_new_invoice_address_to_database_and_returns_AddressDTO()
            => RunTest(
                    service: x => new PersonService(x),
                    prepare: async cmd =>
                    {
                    ...
                    validate: async cmd =>
                    {
                        ...
                        using (var rs = await cmd.ExecuteReaderAsync())
                        {
                            Assert.True(await rs.ReadAsync());
                            Assert.Equal(1, (int)rs["PersonId"]);
                            Assert.Equal(nameof(InvoiceAddress), rs["Type"]);
                            Assert.Equal("Line 1", rs["AddressLine1"]);
                            Assert.Equal("Line 2", rs["AddressLine2"]);
                            Assert.Equal("12345", rs["PostalCode"]);
                            Assert.Equal("City", rs["City"]);
        ...
        public Task Unsets_the_current_address_if_one_exists()
            => RunTest(
                    ...
                    prepare: async cmd =>
                    {
                        await cmd.AddPerson(1, "John", "Doe", new[] {
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(InvoiceAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true)
                        });
                    },
                    execute: async svc =>
                    {
        ...
        public Task Removes_oldest_addresses_if_more_than_3_exists()
            => RunTest(
                    ...
                    prepare: async cmd =>
                    {
                        await cmd.AddPerson(1, "John", "Doe", new[] {
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 1", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(InvoiceAddress), "c/o Jane Doe", "Other Street 1", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 2", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 3", "12345", "The City", "Sweden", true),
                            GetAddress(nameof(DeliveryAddress), "c/o Jane Doe", "Street 4", "12345", "The City", "Sweden", true)
                        });
                    },
                    execute: async svc =>
                    {
